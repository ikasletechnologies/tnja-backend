...
export const login = async (req, res) => {
    ...
    try {
        ...
        if (identifier === adminEmail) {
            ...
            const token = jwt.sign({ userId: "ADMIN", role: "SUPER_ADMIN" }, process.env.JWT_SECRET || "fallback", { expiresIn: "24h" });
            return res.json({
        ...
        if (!user) {
            ...
            if (user)
                role = user.role; // Use the role from the DB (MEMBER, DISTRICT_PRESIDENT, etc.)
        }
        // ── Club ──────────────────────────────────────────────────────────────────
        if (!user) {
...
export const changePassword = async (req, res) => {
    ...
    try {
        ...
        if (role === "PLAYER")
            ...
            return res.status(403).json({ error: "Super Admin password cannot be changed via this endpoint" });
...
export const resetPassword = async (req, res) => {
    ...
    try {
        ...
        for (const m of models) {
            const found = await prisma[m].findFirst({
                where: {
...
export const sendAadhaarOtp = async (req, res) => {
    ...
    try {
        ...
        if (existingStudent)
            return res.status(400).json({ error: "This Aadhaar number is already registered to a Player." });
