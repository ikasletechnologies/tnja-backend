...
export const getApprovedTournaments = async (req, res) => {
    const { role } = req.user;
    if (role !== "CLUB") {
        return res.status(403).json({ error: "Access denied" });
    }
...
export const getRegistrationMessages = async (req, res) => {
...
export const getPlayerTournaments = async (req, res) => {
    ...
    try {
        ...
        const concludedDrawsSet = new Set(concludedDraws.map(d => `${d.tournamentId}_${d.ageGroup}_${d.gender}_${d.weightCategory}`));
        const result = tournaments.map((t) => {
...
export const getCategoryParticipants = async (req, res) => {
...
export const getPlayerPublicMatches = async (req, res) => {
    const { userId, role } = req.user;
    if (role !== "PLAYER" && role !== "STUDENT") {
        return res.status(403).json({ error: "Only players can access this endpoint" });
    }
...
export const getAdminApprovedTournaments = async (req, res) => {
    const { role } = req.user;
    try {
        let whereClause = {};
        if (role === "DISTRICT_PRESIDENT" || role === "DISTRICT_SECRETARY") {
    ...
    try {
        ...
        if (status === "REJECTED") {
            ...
            if (role.startsWith("DISTRICT"))
                dataToUpdate.districtApproval = "REJECTED";
            if (role.startsWith("STATE"))
                dataToUpdate.stateApproval = "REJECTED";
            if (role === "SUPER_ADMIN")
                dataToUpdate.superAdminApproval = "REJECTED";
            if (role === "CEO")
                dataToUpdate.ceoApproval = "REJECTED";
        }
        else {
            if (role.startsWith("DISTRICT"))
...
export const sendTournamentReply = async (req, res) => {
    const { userId, role } = req.user;
    const id = req.params.id;
    const { message } = req.body;
...
export const getTournamentDraws = async (req, res) => {
    const id = req.params.id;
    try {
...
const autoAdvanceWinner = (rounds) => {
            ...
            if (match.status === "COMPLETED" && match.winnerId) {
                ...
                if (nextMatchIdx < nextRound.length) {
                    ...
                    const winnerSlot = {
                        ...
                        regId: isWinnerA ? match.slotA.regId : match.slotB.regId,
                    ...
                    if (roundIdx === rounds.length - 2 && nextRound.length > 1) {
                        const loserSlot = {
                            ...
                            regId: isWinnerA ? match.slotB.regId : match.slotA.regId,
...
export const submitTournamentResults = async (req, res) => {
    ...
    try {
        ...
        const regIds = results.map((r) => r.regId).filter(Boolean);
        const playerIds = results.map((r) => r.playerId).filter(Boolean);
